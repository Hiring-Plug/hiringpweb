...
create table applications (
  ...
  project_id uuid references auth.users not null, -- Project owner receiving application
  job_title text, -- Explicitly store title to avoid complex joins
  resume_url text, -- URL to uploaded PDF
...
create table follows (
  ...
  status text not null check (status in ('following', 'pending', 'connected')),
...
create policy "Users can update their follows/connections."
  on follows for update
  using ( auth.uid() = follower_id or auth.uid() = following_id );

create policy "Users can unfollow/disconnect."
